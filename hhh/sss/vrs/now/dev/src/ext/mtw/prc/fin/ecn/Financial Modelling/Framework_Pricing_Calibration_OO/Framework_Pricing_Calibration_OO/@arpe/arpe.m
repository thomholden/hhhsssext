...
classdef arpe < objectivefuncbase
    properties 
    ...
    methods    
        function y = val(OBJ, x)
            ...
            % calculated form modelprices x
            y = 1/(OBJ.NOpt) * sum(sum(abs(OBJ.weights.*(1 - x ./OBJ.dataMarket))));
        end  
    end
    
    methods
        function OBJ = arpe(dataMarket_,weights_)
