...
clear; clc;
n = 8;                      % N = 2^n grid points
Nex = 10:10:200;            % exercise opportunities
num = size(Nex, 2);
...
NAverage = 5;                   % average for cpu time
d = 6;
...
pricefunccosb = @(x) FFTCOS_B(n, x, Lcos, c, cp, model, S0, t, r, q, strike, vInst, vLong, kappa, omega,rho);
% reference value for an American option using Richardson extrapolation
refa_cos =1/21 * ...
    (64* FFTCOS_B(n, 2^(d+3), Lcos, c, cp, model, S0, t, r, q, strike, vInst, vLong, kappa, omega,rho) ...
    - 56*FFTCOS_B(n, 2^(d+2), Lcos, c, cp, model, S0, t, r, q, strike, vInst, vLong, kappa, omega,rho) ...
    + 14*FFTCOS_B(n, 2^(d+1), Lcos, c, cp, model, S0, t, r, q, strike, vInst, vLong, kappa, omega,rho) ...
    - FFTCOS_B(n, 2^d, Lcos, c, cp, model, S0, t, r, q, strike, vInst, vLong, kappa, omega,rho));

% CONV method
delta = 40;
Lconv = 3;% delta * sqrt(t *(c2^2 + t^2));
% pricing function for a Bermudan option and the CONV method
pricefuncconvb = @(x) FFTCONV_B(n, Lconv, 0.5, cp, model, S0, t, r, q, strike, x, vInst, vLong, kappa, omega,rho);
% reference value for an American option using Richardson extrapolation
refa_conv = 1/21 * ...
   (64* FFTCONV_B(n, Lconv, 0.5, cp, model, S0, t, r, q, strike, 2^(d+3), vInst, vLong, kappa, omega,rho) ...
   - 56*FFTCONV_B(n, Lconv, 0.5, cp, model, S0, t, r, q, strike, 2^(d+2), vInst, vLong, kappa, omega,rho) ...
   + 14*FFTCONV_B(n, Lconv, 0.5, cp, model, S0, t, r, q, strike, 2^(d+1), vInst, vLong, kappa, omega,rho) ...
   - FFTCONV_B(n, Lconv, 0.5, cp, model, S0, t, r, q, strike, 2^(d), vInst, vLong, kappa, omega,rho));
