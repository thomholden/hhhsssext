...
classdef PropertyType
    ...
    methods
        ...
        function tf = isinteger(self)
            tf = any(strcmp({'int8','uint8','int16','uint16','int32','uint32','int64','uint64'}, self.PrimitiveType));
        end
        
        function tf = isfloat(self)
            tf = any(strcmp({'denserealsingle','denserealdouble','densecomplexsingle','densecomplexdouble','sparserealsingle','sparserealdouble','sparsecomplexsingle','sparsecomplexdouble'}, self.PrimitiveType));
        end
        ...
        function tf = isreal(self)
            tf = any(strcmp({'denserealsingle','denserealdouble','sparserealsingle','sparserealdouble'}, self.PrimitiveType));
        end
        
        function tf = is2d(self)
            tf = any(strcmp({'row','column','matrix'}, self.Shape));
        end
        
        function tf = isvector(self)
            tf = any(strcmp({'row','column'}, self.Shape));
        end
            ...
            switch self.Shape
                ...
                case {'row','column'}
                    switch self.PrimitiveType
                        ...
                        case 'cellstr'
                            javavalue = java.lang.String(strjoin(value, newline));
                        case 'logical'
                            if ~isempty(self.Domain)
        ...
        % Casts a MatLab value to the prescribed property type.
            ...
            if iscellstr(value) && strcmp(self.PrimitiveType, 'cellstr') || isobject(value) && strcmp(self.PrimitiveType, 'object') %#ok<ISCLSTR>
                return
            end
            switch self.PrimitiveType
    ...
    methods (Static)
        ...
        %    variable of the general type
            ...
            tf = any(strcmp(type, specifictype));
        end
        ...
        %    a variable of the general shape
            ...
            tf = any(strcmp(shape, specificshape));
        end
        
        function tf = IsInDomain(domain, value)
            if isempty(domain)
                tf = true;
            elseif iscellstr(domain) %#ok<ISCLSTR>
