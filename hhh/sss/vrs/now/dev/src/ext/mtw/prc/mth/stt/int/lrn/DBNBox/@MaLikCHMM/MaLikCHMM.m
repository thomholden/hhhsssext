function [chmm]=MaLikCHMM(varargin);
% [chmm]=MaLikCHMM(LagOp,K,X,obsmodel,options);
%        OR
% [chmm]=MaLikCHMM(LagOp,K,X,obsmodel,options,Y,outlmodel,outloptions);
%
% contructor for maximum likelihood coupled hidden markov models 
%
...
deftrain=struct('cyc',30,'debug',0,'inftype',defhsinfmode, ...
		...
		'outlupdate',repmat(0,1,defNChains),'checkviol',0,...
		'rdisplay',1,'plot',0,'phtime',.1,'tol',1e-5);  
% fields to be copied from HMM structure
defchainstruct=struct('K',defK,'txmodel',[],'txmodelname','TxModels',...
 ...
 otherwise
  if isa(varargin{1},ClassName)
    ...
    if isempty(chains),
	  ...
	  error('Parameter ''chains'' must be of type MaLikHMM');
       end
    end
    ...
    for c=1:chmm.NChains,
      ...
      defchainstructfields=fieldnames(defchainstruct);
