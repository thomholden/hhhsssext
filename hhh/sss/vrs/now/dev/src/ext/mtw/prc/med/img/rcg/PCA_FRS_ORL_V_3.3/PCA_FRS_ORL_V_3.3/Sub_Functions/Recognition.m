function [recp,rectime,outd,recd] = Recognition(ProjectedTestImages,ProjectedImages,TDS,DS,TrNum,TsNum,sno)
...
Euc_dist = [];
for i = 1 : DS
    temp = ( norm( ProjectedTestImages(:,j) - ProjectedImages(:,i) ) )^2; % temperary Variable for storing Euclidean distance value.
...
display(sprintf('\n%d : Recognition Percentage %1.2f%%  and recognition time is %1.3f sec for %d out of %d number of images',sno,recp*100,rectime,TDS,DS));
% save('data.mat','outd','recd')
end