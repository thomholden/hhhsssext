...
%-------------------------------------------------------------------------%
function timer = gtic( gpu )
% Make sure GPU is idle before calling tic
maybeWait(gpu);
timer = tic();

%-------------------------------------------------------------------------%
function elapsedTime = gtoc( timer, gpu )
% Wait for GPU operations to complete then call toc
maybeWait(gpu);
elapsedTime = toc(timer);

%-------------------------------------------------------------------------%
function maybeWait( gpu )
