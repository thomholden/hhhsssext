function[solutionFound,s,Pos] =   NewRepair(sizeChain,Pos,pos,moves,s,vector)
% NewRepair is an auxiliary function that modifies a sequence configuration to avoid self-intersections
    ...
    if (size(s,2) == sizeChain)
     ...
     while (solutionFound==0  & i<=4)
      if(i==1 || moves(pos,i) ~= vector(pos))      
       ...
       [solutionFound,new_s,Pos] = NewRepair(sizeChain,Pos,pos+1,moves,new_s,vector);
      end
      i = i + 1;
      if (solutionFound==1)
         s = new_s;
