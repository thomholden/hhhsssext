...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval,perf] = deopt(fname,S_struct)
...
%----Output section----------------------------------------------------------
  ...
  I_iter = I_iter + 1;
  perf.xk = [perf.xk FVr_bestmem'];
  perf.fk = [perf.fk S_bestval.FVr_oa(1)];
end %---end while ((I_iter < I_itermax) ...

perf.iter = I_iter;
perf.itertotal = I_iter*I_NP;
