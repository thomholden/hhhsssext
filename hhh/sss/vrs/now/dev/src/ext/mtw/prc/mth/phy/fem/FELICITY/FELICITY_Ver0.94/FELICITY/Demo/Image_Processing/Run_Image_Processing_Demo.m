...
Num_Opt_Iter = 80; % number of optimization iterations

% user parameters
PARAM.init_step_size = 1.0; % intial step size to try
PARAM.mu     = 0.01; % curve length penalty
PARAM.Cin    = 0.0; % inside pixel value (dark)
PARAM.Cout   = 1.0; % outside pixel value (light)
PARAM.W_L2   = 1.0; % L^2 weight
PARAM.W_H1   = PARAM.W_L2/20; % H^1 weight
...
for ind=1:Num_Opt_Iter
    [New_Vtx, V_vec, END_OPT] = Image_Proc_Demo_Optimization_Step(P1_Mesh_DoFmap, Curve(ind).Vtx, Image_Data, PARAM);
