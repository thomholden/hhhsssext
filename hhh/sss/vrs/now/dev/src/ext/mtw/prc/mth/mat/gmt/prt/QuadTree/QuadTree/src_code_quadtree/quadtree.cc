...
private:
    ...
    // counter for making sure the print-out lists all of the points
...
void TR::Setup_Levels(const UINT_type Max_Levels)
{
...
/* Test whether x is in the interval [a,b).
   Note: must have a <= b. */
inline bool in_interval(const pt_type& x, const pt_type& a, const pt_type& b)
{
    if ( (x >= a) && (x < b) )
        return true;
    else
    ...
    for (it = temp_indices.begin(); it != temp_indices.end(); ++it)
        ...
        if ( in_interval(pt_coord[0], Min_X, Max_X) &&
            ...
            if (!( in_interval(pt_coord[0], NN_Min_X, NN_Max_X) &&
                ...
                mexPrintf("   try setting a large value for Max_Levels (<= 50).\n");
                mexErrMsgTxt("STOP!");
                }
            }
        }
...
void TR::Print_Node_Info(trCell* node)
{
    if (node!=NULL)
        {
        mexPrintf("Level: %d   |  Side Length: %1.15f\n",node->level,node->rt_v[0] - node->lb_v[0]);
...
void TR::print_leaf(trCell* leaf)
{
    ...
    mexPrintf("Level: %d   |  Side Length: %1.15f\n",leaf->level,leaf->rt_v[0] - leaf->lb_v[0]);
...
/* clamp the value x to the range [a, b]
   Note: must have a <= b. */
inline pt_type TR::clamp(const pt_type& x, const pt_type& a, const pt_type& b)
{
    if (x <= a)
        return a;
    else if (x >= b)
...
inline UINT_type my_trunc(const pt_type& VAL)
{
