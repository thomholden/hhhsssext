...
% and provide comparison with traditional discrete Fourier transform (DFT) output.
% EDFT call line is [F,S,Stopit]=edft(X,N,1,W),
...
% - complex exponent at frequency 0.137+1/(4*K), magnitude 1, where K=64 is length
%   of sequence X,
...
% So, the true spectrum of DEMO sequence (without noise part) is ploted in red color and
% looks like greeting 'HI', where 'I' actually are two close peaks at normalized frequencies
% 0.137 and 0.137+1/(4*K). In the first, demoedft.m plot real parts of the input sequences:
% - Subplot(311) display DEMO data X1 consisting of 64 samples;
% - Subplot(312) outputs an example of sparse data X2, where 32 randomly taken samples in X1
%   replaced by NaN;
% - Subplot(313) shows gapped data X3, where two gaps by 16 NaN each inserted in X1.
% NaN are not displayed in sublopts 312 and 313. 
% To proceed you should select one of these sequences - enter [1] to see DEMO for 
% 64 uniformly sampled data or select [2] or [3], where only 32 available samples will be
% processed by DFT and EDFT algorithms. In the next, fifteen (I=15) EDFT iterations
% are performed starting with edft(X,N,1,ones(1,N))=fft(X,N). 
% You will be asked to strike any key after 1,2,3,5 and 10 iteration.
% Plots in blue colour are equal to traditional DFT analysis and can be used for comparison
% with EDFT outputs (green colour). To resolve two close sinusoids, EDFT should
% increase resolution at least 4 times in comparison with traditional DFT.
...
%   frequencies range [-0.5 0.5[. Note, that results of the first iteration coincide
%   to periodogram: 10*lg(abs(fft(X,N).^2/N)).
% - Subplot(222) display Power Spectrum as 10*log10(abs(S).^2). That plot indicates the power
%   of sinusoids in data X.
% - Subplot(413) plot division F./S/K titled as Relative frequency resolution and expose how
%   the frequency resolution of EDFT algorithm is changed along the frequency axes in respect
%   to DFT analysis. The first iteration plot F./S/K=const and showing that traditional DFT
%   have equal resolution for all frequencies in range [-0.5 0.5[. Next iterations discover
%   that the relationship F./S/K for EDFT analysis is not that simple as for traditional DFT.
%   Although sum(F./S/K)=N and remains constant for all iterations, EDFT have ability to
%   increase resolution (in plot appears values >1) around the powerful narrowband
%   components (sinusoids) and decrease resolution (in plot appears values <1) at
%   frequencies where data X have weak power components. EDFT is called as high
%   resolution method and that's true, but with the following remark - EDFT keeping the
%   same 'summary' resolution as traditional DFT or, in other words, squares under curves
%   (F./S)/K for traditional DFT (blue colour) and EDFT (green colour) analysis are equal.
%   Maximum frequency resolution is limited by value of division N/K. For example, if K=64
%   and N=512 then EDFT can potentially improve frequency resolution512/64=8 times.
% - Subplot(414) plot real parts of reconstructed sequences obtained as result of inverse
%   fast Fourier transform to outputs of DFT and EDFT - ifft(dft_out) and ifft(F),
%   correspondingly. It is well known that reconstruction of data by applying ifft(fft(X,N))
%   with N>length(X), return sequence where initial data X padded with zeros to length N.
%   That result is displayed for the first iteration. The next EDFT iterations provide
%   ability to obtain reconstructed sequence consisting of initial data X values plus
%   non-zero extrapolation of X to length N. As shown in subplot 414 data X are
%   extrapolated in both directions - backward and forward. But in case of NaN, data
%   also are interpolated and 32 NaN are replaced with reconstructed values.
%
% Vilnis Liepins (vilnislp@gmail.com)
% Reference:	V. Liepin'sh, "An algorithm for evaluation a discrete Fourier transform for
%		incomplete data", Automatic control and computer sciences,Vol.30, No.3,
%		pp.27-40, 1996 (ISSN0132-4160).
...
% uniformly spaced time and frequency vectors
	...
	fn=[-ceil((N-1)/2):floor((N-1)/2)]/N; % uniform normalized frequency set, [0.5,0.5[
...
% Generate X2 - nonuniformly sampled EDFT input: sparse sequence (K/2 samples, K/2 NaN)
	...
	X2=X1;XX2=X1;
	KNaN=(1:2:MNaN*2)+round(rand(1,MNaN)); % KNaN- indicate NaN in input sequence
	X2(KNaN)=NaN*ones(1,MNaN); % sparse sequence with NaN (EDFT input)
	XX2(KNaN)=zeros(1,MNaN);	% sequence where NaN replaced with zeros (FFT input)
...
% K/2 of K samples replaced with NaN to demonstrate EDFT performance for incomplete data.
	...
	if DEMO==1,
	X=X1;XX=X1;KP=K;h=2;		% X=X1 - uniform data
        	elseif DEMO==2,
	X=X2;XX=XX2;KP=KK;h=3;		% X=X2 - sparse data
	elseif DEMO==3,
	X=X3;XX=XX3;KP=KK;h=4;		% X=X3 - gapped data
	else
...
% Calculate outputs for DFT (periodogram) used in 4 plottings.	
	...
	sub2_PS=20*log10(fftshift(abs(dft_out/KP)));
...
% Plots Reconstructed Data in subplot414.
	...
	xlabel('Sample number')
	ylabel('real(ifft(F))')
title(['Sequence X',int2str(DEMO),': True (red) and reconstructed by EDFT (green, iter.',int2str(it),') and DFT (blue)'])
...
disp(['Calculate [F,S]=edft(X',int2str(DEMO),',',int2str(N),',',int2str(it),') and ifft(F).']);
	end