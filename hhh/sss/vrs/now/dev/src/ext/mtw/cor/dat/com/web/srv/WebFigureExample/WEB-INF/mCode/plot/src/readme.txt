 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_plot.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* plot.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* plot.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* plot (application created by MATLAB Compiler)
		* run_plot.sh (shell script that can be run 
	...
	All Platforms:            
		* plot.dll (shared library, file extension 
		  varies by platform)
		* plot.h (library header file)
		* plot.ctf (Component Technology File archive,
	...
	* plot.xml (documentation files)
	* plot.pdb (program database file - if 
	  DEBUG option selected)
	* plot.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* plot.jar (Java class files and compiled M-code)
	...
	* plot.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* plot.xla (Optionally created and placed in
	  C:\Projects\workspace\WebFiguresExample\m\plot\src directory by the user)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
