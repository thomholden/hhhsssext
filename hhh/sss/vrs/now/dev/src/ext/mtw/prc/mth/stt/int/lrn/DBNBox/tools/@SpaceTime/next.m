function [spi]=next(spi,varargin)
%
%  spi=next(spi)
%  sets the  space-time index to next in fashion defined by its type
...
spi=add(spi,incr);
spi.elcntr=spi.elcntr+1;

function [spi]=add(varargin)
%  spi=spi+1;  or 
%  spi=spi+[1 1];			
  ...
  switch spi.type
   case 'linear'
    if prod(size(d))==1,		% d is just a real
      spi.tc=spi.tc+d;
      if spi.tc<1 |  spi.tc>spi.TC, spi.ovfl=1; end;
   ...
   case 'tcyclic'
    % This section implements time first cyclic behavious or the index
    ...
    if l>spi.TC, spi.ovfl=1; end; % check overflow
    l=spi.TC*(mod(l,spi.TC)==0)+mod(l,spi.TC);
    spi.tc=l;
    [spi.ti,spi.ch]=ind2sub(L,l);
   case 'ccyclic'
    % This section implements chain first cyclic behavious of the index
    L=[spi.C spi.T];
    l=mysub2ind(L,spi.ch,spi.ti);	% current index
    if prod(size(d))==1,		% d is just a real
      l=l+d;
    else
      d(1)=d(1)+spi.ti;			% d is vector [ti,ch]
      ...
      d(1)=spi.T*(mod(d(1),spi.T)==0)+mod(d(1),spi.T);
      d(2)=spi.C*(mod(d(2),spi.C)==0)+mod(d(2),spi.C);
      l=mysub2ind(L,d(2),d(1));
    end
    if l>spi.TC, spi.ovfl=1; end; % check overflow
    l=spi.TC*(mod(l,spi.TC)==0)+mod(l,spi.TC);
    [spi.ch,spi.ti]=ind2sub(L,l);
    spi.tc=mysub2ind(fliplr(L),spi.ti,spi.ch);
   otherwise
