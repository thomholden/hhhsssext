...
if isempty(chschain.Gamma) & isempty(chschain.Xi) 
  for n=1:length(B.block),
    L = B.block(n).L;
    [Gamma,Xi]=initsingchabel(K,chschain.LagOpSpec,L);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Gamma,Xi]=initsingchabel(K,LagOpSpec,B)
...
for c=1:C,				% initialise
  ...
  initgam=rand(size(initgam));
  initgam=rdiv(initgam,rsum(eps+initgam));
  Gamma=cat(2,Gamma,num2cell(initgam',1)');
  sv=[K(c),K(LagOpSpec{c}(2,:))];	% size vector
