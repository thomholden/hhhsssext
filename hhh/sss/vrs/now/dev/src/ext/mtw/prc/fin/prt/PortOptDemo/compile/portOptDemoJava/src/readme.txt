 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_portOptDemoJava.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* portOptDemoJava.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* portOptDemoJava.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* portOptDemoJava (application created by MATLAB Compiler)
		* run_portOptDemoJava.sh (shell script that can be run 
	...
	All Platforms:            
		* portOptDemoJava.so (shared library, file extension 
		  varies by platform)
		* portOptDemoJava.h (library header file)
		* portOptDemoJava.ctf (Component Technology File archive,
	...
	* portOptDemoJava.xml (documentation files)
	* portOptDemoJava.pdb (program database file - if 
	  DEBUG option selected)
	* portOptDemoJava.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* portOptDemoJava.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* portOptDemoJava_0_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* portOptDemoJava.jar (Java class files and compiled M-code)
	...
	* portOptDemoJava.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* portOptDemoJava.xla (Optionally created and placed in
	  /home/sholden/Business/Jacobix/Dev/Pascal-Linux/ASSET/dev/queued/matlab/java/PortOptDemo/compile/portOptDemoJava/src directory by the user)  
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
