% Demonedft.m demonstrates performance of NEDFT algorithm by iterations
% in case of nonuniform sampling and provide comparison with traditional nonuniform
% discrete Fourier transform (DFT) output. NEDFT call line is
...
% - complex exponent at frequency 0.137+1/(4*K), magnitude 1 (K=64), 
%   where K=64 is length of sequence X,
...
% Number of samples K=64 (uniform and nonuniform sequences) or KK=32 (nonuniform sequence,
% 1/2 of samples missing). So, the true spectrum of DEMO sequence (without noise part)
% looks like greeting 'HI', where 'I' actually are two close peaks at normalized frequencies
% 0.137 and 0.137+1/(4*K). In the first, demonedft.m plots real parts of the input sequences:
...
% To proceed you should select one of these sequences - enter [1] to see DEMO for 64
% uniformly sampled data or select [2] or [3], where 64 or 32 nonuniformly sampled data
% will be processed by DFT and NEDFT algorithms. In the next, fifteen (I=15) NEDFT iterations
% are performed. You will be asked to strike any key after 1,2,3,5 and 10 iteration.
% Plots in blue colour are equal to DFT analysis and can be used for comparison with NEDFT
% output (green colour). To resolve two close sinusoids (at 1/4 of Nyquist), NEDFT should
% increase resolution at least 4 times (in comparison with DFT, see Subplot(413)).
%
% NEDFT and DFT outputs are displayed in four subplots:
% - Subplot(221) show Power Spectral Density as 10*log10(abs(F).^2/N)) in normalized
%   frequencies range [-0.5 0.5[.
% - Subplot(222) display Power Spectrum as 10*log10(abs(S).^2). The plot indicate power of
%   sinusoids in X.
% - Subplot(413) plot division F./S/K titled as Relative frequency resolution and expose how
%   the frequency resolution of NEDFT algorithm is changed along the frequency axes in
%   respect to DFT analysis. The first iteration showing that DFT and NEDFT analysis have
%   almost equal resolution for all frequencies in range [-0.5 0.5[. During the next NEDFT
%   iterations we can discover that the relationship F./S/K for NEDFT analysis is not that
%   simple as for DFT. Although sum(F./S/K)=N and remains constant for all iterations, 
%   NEDFT have ability to increase resolution around the powerful narrowband components
%   (sinusoids) and decrease resolution at frequencies where data X have weak power
%   components. NEDFT is called as high resolution method and that's true, but with the
%   following remark- NEDFT keeping the same 'summary' resolution as DFT or, in other
%   words, squares under curves for DFT (blue colour) and NEDFT (green colour) analysis
%   are equal. The maximum frequency resolution is limited by value of division N/K. 
%   For example, if K=64 and N=512, then NEDFT can potentially improve frequency
%   resolution 512/64=8 times.
% - Subplot(414) plot real parts of reconstructed sequences obtained as result of inverse
%   fast Fourier transform to outputs of DFT and NEDFT - ifft(fftshift(dft_out)) and
%   ifft(fftshift(F)), correspondingly. It is well known that reconstruction of data by
%   applying ifft(fft(X,N)) for N>length(X), return sequence where initial data X are 
%   padded with zeros to length N. That result is true also for DFT and for the first
%   iteration of NEDFT. The next iterations are showing ability of NEDFT to obtain
%   reconstructed data consisting of input sequence X values plus non-zero 
%   extrapolation of X to length N. As shown in subplot 414, sequence X are 
%   extrapolated in both directions - backward and forward. Also, for nonuniformly
%   sampled input sequences [X2] and [X3], the reconstructed data are re-sampled
%   at uniform time set tn=1:N.
%
...
% Generate uniformly and nonuniformly spaced time, frequency, noise vectors and initial phases of sinusoids.
	...
	tk1=1:K;			% uniform (K samples, sampling period T=1)
	tkn2=[1:K]+0.5*(rand(1,K)-0.5); % nonuniform (K, mean sampling period Ts=1)
	tkn3=tkn2(1:2:K); 	% sparse nonuniform (K/2, mean sampling period Ts=2)
	fn=[-ceil((N-1)/2):floor((N-1)/2)]/N;% uniform frequency set, [-0.5,0.5[
	tn=1:N;			% uniform (N, sampling period T=1)
	ti1=tk1-K/2-0.5;		% uniform centerred time for pulse
	tin2=tkn2-K/2-0.5;		% nonuniform centerred time for pulse
	cno=sigma*(randn(1,K)+i*randn(1,K))/2;	% complex noise
	ph1=2*pi*rand;		% phase of sinusoid at frequency fc
	ph2=2*pi*rand;		% phase of complex exponent at frequency fc
...
% Selecting of sequence for NEDFT input and calculating DFT output
	...
	if DEMO==1,
	    X=X1;tk=tk1;KK=K;h=2;		% X=X1 - uniform data (K samples)
	    E=exp(-i*2*pi*tk1.'*fn);		% E - complex exponents matrix
	    dft_out=X*E; 			% Calculate DFT output
	elseif DEMO==2,
	    X=X2;tk=tkn2;KK=K;h=3;		% X=X2 - nonuniform data (K samples)
	    E=exp(-i*2*pi*tkn2.'*fn);		% E - complex exponents matrix
	    dft_out=X*E; 			% Calculate DFT output
	elseif DEMO==3,
	    X=X3;tk=tkn3;KK=K/2;h=4;	% X=X3 - sparse nonuniform data (K/2 samples)
	    E=exp(-i*2*pi*tkn3.'*fn);		% E - complex exponents matrix
	    dft_out=X*E; 			% Calculate DFT output
	else
	   disp('...end of NEDFT DEMO.')	% End DEMO if other valid key entered
	   return
	end
...
% Plots True and Reconstructed data in subplot414.
	...
	xlabel('Sample number')
	ylabel('real(ifft(F))')
title(['Sequence X',int2str(DEMO),': True (red) and reconstructed by NEDFT (green, iter.',int2str(it),') and DFT (blue)'])
...
disp(['Calculate [F,S]=nedft(X',int2str(DEMO),',tk,fn,',int2str(it),') and ifft(F).'])
	end