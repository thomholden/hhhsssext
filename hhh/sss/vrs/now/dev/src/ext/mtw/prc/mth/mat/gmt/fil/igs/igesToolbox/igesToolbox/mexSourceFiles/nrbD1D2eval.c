...
static void nrbD1D2eval(int order, int ncp, mxArray *nrbStruct, mxArray *nrbDerStruct, mxArray *nrbDer2Struct, double *paramValuePtr, double *evalPnt, double *evalDer, double *evalDer2, double *bspPnts, double *left, double *right, double *N) {
    ...
    if (paramValuePtr[0]<=mxGetPr(mxGetField(nrbStruct, 0, "knots"))[deg]){
        ...
        if (order>2){
            ...
            evalDer2[0]=(cp[0]-2*weights*(evalDer[0])-(cp[3])*(evalPnt[0]))/weightsPnts;
            evalDer2[1]=(cp[1]-2*weights*(evalDer[1])-(cp[3])*(evalPnt[1]))/weightsPnts;
            evalDer2[2]=(cp[2]-2*weights*(evalDer[2])-(cp[3])*(evalPnt[2]))/weightsPnts;
        }
        else{
            evalDer2[0]=0.0;
            evalDer2[1]=0.0;
        ...
        if (order>2){
            ...
            evalDer2[0]=(cp[fourncp-4]-2*weights*(evalDer[0])-(cp[fourncp-1])*(evalPnt[0]))/weightsPnts;
            evalDer2[1]=(cp[fourncp-3]-2*weights*(evalDer[1])-(cp[fourncp-1])*(evalPnt[1]))/weightsPnts;
            evalDer2[2]=(cp[fourncp-2]-2*weights*(evalDer[2])-(cp[fourncp-1])*(evalPnt[2]))/weightsPnts;
        }
        else{
            evalDer2[0]=0.0;
            evalDer2[1]=0.0;
