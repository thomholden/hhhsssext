		 ...
		 const int nrhs, const mxArray *prhs[])
{
  ...
  
  {
	...
	Cl[j*(m+1)]=es+js;
      ...
      dpotrf_(uplo, &m, C, &m, &info);
      dpotri_(uplo, &m, C, &m, &info);
    ...
    /* R = Cl'*invC */
    dsymm_(sidel, uplo, &m, &m, &done, Cl, &m, C, &m, &dzero, R, &m);
    /* y=R*b */
    dgemv_(trans, &m, &m, &done, R, &m, b, &one, &dzero, y, &one);
    /* lcov=lcov-R*Cl */
    dsymm_(sider, uplo, &m, &m, &dmone, Cl, &m, R, &m, &done, lcov, &m);
    /* lcov=chol(lcov) */
    dpotrf_(uplo, &m, lcov, &m, &info);
    /* ly=lcov'*ly (remember that ly contains the random numbers) */
    dtrmv_(uplo, trans, diag, &m, lcov, &m, ly, &one);
    /* ly = ly + y */
    for (i=0;i<m;i++) 
       ly[i]+=y[i]; 
