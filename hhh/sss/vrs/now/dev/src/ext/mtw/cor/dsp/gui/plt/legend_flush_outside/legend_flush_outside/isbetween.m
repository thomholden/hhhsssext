...
% April 29, 2011        Allow "b" to be a vector.
% June 29, 2011         Handle open-ended (+/-Inf) range.
% June 25, 2012         If c < b, then the answer is empty.
    ...
    if ~exist('b', 'var') || any(~isnumeric(b))
        errordlg('Input "b" is missing or non-numeric.', mfilename);
        return
    end

    %   Allow "b" to be a vector.
    if (length(b) == 2) && ~exist('c', 'var')
        c = b(2);
        b = b(1);
    end
    ...
    for a_index = 1:length(a)
        if inclusive
            boolean(:, a_index) = (a(a_index) >= (b + threshold_value)) & (a(a_index) <= (c - threshold_value));
        else
            boolean(:, a_index) = (a(a_index) >  (b + threshold_value)) & (a(a_index) <  (c - threshold_value));
        end
    end
