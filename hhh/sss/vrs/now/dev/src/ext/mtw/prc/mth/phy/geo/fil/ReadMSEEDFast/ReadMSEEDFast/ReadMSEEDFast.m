...
%
    ...
    rawPacketMatrix = reshape(typecast(uint8(raw),'uint8'),4096,[])';
    ...
    for h=1:numel(stations)
        for k=1:numel(channels)
                ...
                signalMatrix=DecodeSignal(encodedSignalMatrix,ENCODING,isLittleEndian);
...
function [trace] = DecodeSignal(encodedSignalMatrix, encodingFormat,isLittleEndian)
...
%             % --- decoding format: IEEE double precision floating point
        ...
        case {10,11,19}
            ...
            switch steim
               ...
               if mod(size(ddd,1),1008*maxValuesInChunk) ~= 0
                   ddd = [ddd; nan*zeros(1008*maxValuesInChunk-mod(size(ddd,1),1008*maxValuesInChunk),1)];
               end

               blockMatrix=reshape(ddd,1008*maxValuesInChunk,[]);
...
function [encoding] = ReadBlockets(raw,nFolowingBlocketts,blockettBeginOffset,isLittleEndian)
