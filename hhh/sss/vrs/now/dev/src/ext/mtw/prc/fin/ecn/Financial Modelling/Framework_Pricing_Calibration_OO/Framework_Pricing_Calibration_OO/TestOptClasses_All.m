...
modelname = 'nig';
optmethod = 'sqp'; % 'de', 'nelder', 'fmincon', 'sqp', 'levmar'
fftmethod = 'cos'; % 'bs', 'cos', 'lewis' 'cm'
...
P_struct.t_star = 0.0;                                            % Forward Start 
...
elseif(strcmp(ofunc,'rmse'))
    ...
    error('Unknown objective function %s',ofunc);
end
...
if(strcmp(modelname,'vg'))
    ...
    mpar.cgm = true;
    mbd.setmodelbuilder(vgmodelbuilder(dstruct));

elseif(strcmp(modelname,'vgcir'))
    params = [5.1151 12.4954 1.3685 1.2329 0.6498 1.4334];
    mpar.c = params(1); mpar.g = params(2); mpar.m = params(2);
    mpar.kappa = params(4); mpar.eta = params(5); mpar.lambda = params(6);
    mpar.usec = true; mpar.useg = false; mpar.usem = true;
    mpar.usekappa = true; mpar.useeta = false; mpar.uselambda = false;
    mbd.setmodelbuilder(vgcirmodelbuilder(dstruct));
...
elseif(strcmp(modelname,'vggou'))
    ...
    mpar.kappa = params(4); mpar.eta = params(5); mpar.lambda = params(6);
    mpar.usec = true; mpar.useg = false; mpar.usem = true;
    mpar.usekappa = true; mpar.useeta = true; mpar.uselambda = false;
    mbd.setmodelbuilder(vggoumodelbuilder(dstruct));
    
elseif(strcmp(modelname,'nig'))
    params= [4 2 1];
    mpar.alpha = params(1); mpar.beta = params(2); mpar.delta = params(3);
    mpar.usealpha = true; mpar.usebeta = true; mpar.usedelta = false;
    mbd.setmodelbuilder(nigmodelbuilder(dstruct));
    
elseif(strcmp(modelname,'nigcir'))
    params = [4	3 1 0.3 0.3 0.3];
    mpar.alpha = params(1); mpar.beta = params(2); mpar.delta = params(3);
    mpar.kappa = params(4); mpar.eta = params(5); mpar.lambda = params(6);
    mpar.usealpha = true; mpar.usebeta = true; mpar.usedelta = true;
    mpar.usekappa = true; mpar.useeta = true; mpar.uselambda = true;
    mbd.setmodelbuilder(nigcirmodelbuilder(dstruct));

elseif(strcmp(modelname,'niggou'))
    params = [4	3 1 0.9397 1 3];
    mpar.alpha = params(1); mpar.beta = params(2); mpar.delta = params(3);
    mpar.lambda = params(6); mpar.a = params(4); mpar.b = params(5);
    mpar.usealpha = 1; mpar.usebeta = 1; mpar.usedelta = 1;
    mpar.uselambda = 1; mpar.usea = 1; mpar.useb = 1;
    mbd.setmodelbuilder(niggoumodelbuilder(dstruct));
...
elseif(strcmp(modelname,'mj'))
    ...
    mpar.alpha_j = params(2); mpar.sigma_j = params(3); mpar.lambda = params(4);
        mpar.usesigma = 1; 
    mpar.usealpha_j = 1; mpar.usesigma_j =1; mpar.uselambda = 1;
    mbd.setmodelbuilder(mjmodelbuilder(dstruct));
    
elseif(strcmp(modelname,'bs'))
    params = 0.25;
    mpar.sigma = params(1);
    mpar.usesigma = 1;
    mbd.setmodelbuilder(bsmodelbuilder(dstruct));
...
elseif(strcmp(modelname,'heston'))
    ...
    mpar.kappa = params(3); mpar.omega = params(4); mpar.rho = params(5);
        mpar.usev0 = 1; mpar.usetheta = 1; 
    mpar.usekappa = 1; mpar.useomega = 1; mpar.userho =1;
    mbd.setmodelbuilder(hestonmodelbuilder(dstruct));
...
elseif(strcmp(modelname,'bates'))
    ...
    mpar.lambda = params(6); mpar.muj = params(7); mpar.sigmaj = params(8);
        mpar.usev0 = 1; mpar.usetheta = 1; 
    mpar.usekappa = 1; mpar.useomega = 1; mpar.userho = 1;
    mpar.uselambda = 1; mpar.usemuj = 1; mpar.usesigmaj = 1;
    mbd.setmodelbuilder(batesmodelbuilder(dstruct));
else
...
elseif(strcmp(fftmethod,'lewis'))
    ...
    error('Unknown pricer %s',fftmethod);
end
...
elseif(strcmp(modelname,'nigcir'))
    lb = [5	-15	0 0 0 0]; ub = [15 15 1 5 5 5];
elseif(strcmp(modelname,'niggou'))
    lb = [0	-5	0 0 0 0]; ub = [15 15 1 5 5 5];
elseif(strcmp(modelname,'mj'))
...
S_struct.lb = lb(model.usevec == true);
S_struct.ub = ub(model.usevec == true);
S_struct.start = params(model.usevec==true);
S_struct.I_plotting = 0;

P_struct.cfunc = [];

if strcmp(optmethod,'de') 
    S_struct.I_D = sum(model.usevec==true);
    S_struct.I_NP = 20;
    S_struct.F_weight = .7;
    S_struct.F_CR = .8;
    ...
    if(strcmp(modelname,'nig'))
        P_struct.cfunc = @(x)nig_constfun_de(x,model);
    end
end
...
if strcmp(optmethod,'sa')
    ...
    S_struct.options = options;
end

if strcmp(optmethod,'levmar')
    % options levenberg marquardt
    %--------------------------------------------------------------------
    opts.tau = 1e-3; % determines acceptance of a L-M step
    opts.tolg = 1e-9; % convergence tolerance for gradient
    opts.tolx = 1e-12; % convergence tolerance for parameters
    opts.maxeval = 400; % maximum number of iterations
    
    S_struct.options = opts;
end

if strcmp(optmethod,'fmincon')
    options = optimset('TolFun', 1.0e-9, 'TolCon', 1.0e-9, 'DiffMaxChange', 5, 'DiffMinChange', 1);
    S_struct.options = options;
end

if strcmp(optmethod,'nelder') 
    options = optimset('MaxFunEvals',1e+10,'MaxIter',1e+6);
    S_struct.options = options;
end        


if strcmp(optmethod,'sqp') 
    if(strcmp(modelname,'nig'))
        P_struct.cfunc = @(x)nig_constfun_sqp(x,model);
    end
end

if strcmp(optmethod,'fmincon') 
    if(strcmp(modelname,'nig'))
        P_struct.cfunc = @(x)nig_constfun_fmincon(x,model);
    end
end

[nrR,nrC] = size(P_struct.dataOpt);
P_struct.NrOpt = nrR*nrC;

if strcmp(optmethod,'levmar')
    objfun = @(x)reshape(fftpricer.price(P_struct.dataT, ...
                  P_struct.t_star, P_struct.S0, P_struct.d, P_struct.df, ...
                  x, P_struct.dataK, P_struct.dataOptType)-P_struct.dataOpt,...
                  P_struct.NrOpt,1);
else
    objfun = @(x)P_struct.ofunc.val(fftpricer.price(P_struct.dataT, ...
                  P_struct.t_star, P_struct.S0, P_struct.d, P_struct.df, ...
                  x, P_struct.dataK, P_struct.dataOptType));

end

constfun = P_struct.cfunc;

if strcmp(optmethod, 'sqp')
    sqp = optsqp(S_struct,objfun,constfun);
    [x,f,perf] = sqp.optimize();
    clear sqp;
elseif strcmp(optmethod , 'de')
    diffevo = optde(S_struct,objfun,constfun);
    [x,S_y,I_nf] = diffevo.optimize();
    f = S_y.FVr_oa;
    clear diffevo;
elseif strcmp(optmethod, 'nelder')
    nm = optneldermead(S_struct,objfun);
    [x, f] = nm.optimize();
    clear nm;
elseif strcmp(optmethod, 'sa')
    annealer = optsa(S_struct, objfun);
    [x,f] = annealer.optimize();
    clear annealer;
elseif strcmp(optmethod, 'fmincon')
    fminc = optfmincon(S_struct,objfun,constfun);
    [x, f] = fminc.optimize();
    clear fminc;
elseif strcmp(optmethod, 'levmar')
    levmar = optlevmar(S_struct,objfun);
    [x, ~] = levmar.optimize();
    f = P_struct.ofunc.val(fftpricer.price(P_struct.dataT, ...
                  P_struct.t_star, P_struct.S0, P_struct.d, P_struct.df, ...
                  x, P_struct.dataK, P_struct.dataOptType));
    clear levmar;
else
    error('Unknown optimizer %s',optmethod);
end
...
PlotPrices(P_struct,fftpricer,x)
