...
%  assume axes are added on right, then left, then right, etc.
  ...
  h_ax = axes('Units', get(cah, 'Units'), 'position',[xpos, cpos(2), cpos(3)*.015, cpos(4)]);
...
%  rescale all y-values
  y = get(hplt, 'YData'); %varargin{2};
  y = (y-yl2(1))./(yl2(2)-yl2(1)).*(yl(2)-yl(1))+yl(1);
  
  varargin{2} = y;  % This is what prevents us from using TimeSeries directly.. how to fix?
  axes(cah)
...
% set(cah,'userdata',axh);  
  ...
  %  or dissappear. Edit LP 14/9/2014, resize with a top-down approach
  %  rather than heuristically so that the figure can be resized or zoomed
  %  later on
  
% HERE IS THE ORIGINAL AA5 CODE (LP)
  axhand = cah;
  set(cah, 'units', 'pixels');
  postot(1,:) = get(cah,'position');
  for I = 1:length(axh)
    axhand(I+1) = axh{I}(1);
    set(axhand(I+1), 'units', 'pixels');
...
setaddaxisdata(cah, get(cah, 'YLim'), 'OriginalZoomYLim');
set(zoom(hfig),'ActionPostCallback',@(x,y)  aadzoomendfcn(x, y));
set(zoom(hfig),'ActionPreCallback',@(x,y) aadzoomstartfcn(x, y));
set(pan( hfig),'ActionPostCallback',@(x,y)  aadzoomendfcn(x, y));
set(pan( hfig),'ActionPreCallback',@(x,y) aadzoomstartfcn(x, y));
zoom reset;
% Use custom datatips
dcm = datacursormode(hfig);
set(dcm, 'Enable', 'on', 'UpdateFcn', @(h, event) aadDataTipText(h, event));

% Set the figure resize callback if needed...
if isempty(get(hfig, 'ResizeFcn'))
    set(hfig, 'ResizeFcn', @(x,y) aadwindowresizefcn(x,y));
end
aad_hax = getaddaxisdata(hfig, 'ActiveWindowResizeH');
aad_hax = union(aad_hax, cah);
setaddaxisdata(hfig, aad_hax, 'ActiveWindowResizeH')
end