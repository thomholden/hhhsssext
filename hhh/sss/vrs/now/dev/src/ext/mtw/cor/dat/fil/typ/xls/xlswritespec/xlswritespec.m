function [success,message]=xlswritespec(file,data,sheet,range,params)
% XLSWRITESPEC Stores numeric array or cell array in Excel workbook.
%   [SUCCESS,MESSAGE]=XLSWRITESPEC(FILE,ARRAY,SHEET,RANGE) writes ARRAY to the Excel
%   workbook, FILE, into the area, RANGE in the worksheet specified in SHEET.
%   FILE and ARRAY must be specified. PARAMS is an optional parameter
%   to improve formatting of the data, you will not have long hideous numbers if you
%   set the formatting right for example. If either FILE or ARRAY is empty, a
...
%   See NOTE 1.
%
%   [SUCCESS,MESSAGE]=XLSWRITESPEC(FILE,ARRAY) writes ARRAY to the Excel workbook,
%   FILE, starting at cell A1 of the first worksheet. The return values are as
%   for the above example. Graceful degradation is provided that the script
%   does not crash instead no formatting is performed.
%
...
%               'A1' notation.
%         params: Specifies the format of the cell, currently you can set the color
%                 of the text, the kind of text formatting you would like to impose
%                 like number or text. Whether you would like to bold such text and
%                 of course the exact font you would like to use, I generally use 
%                 verdana.
%
%   RETURN PARAMETERS:
...
%   
%     The params parameter is to allow for special formatting of a particular cell for example
%     to change the color of the text and so on . The trick I use here is to add certain other
%     properties to the selection before I write into the stuff.
%     
%     setting the params as a part of a struct before you pass it in helps
%     params.Bold = 'false';
%     params.FontSize = 10;
%     params.FontName = 'Verdana';
%     params.ColorIndex = 14;
%     params.NumberFormat = '0.000';
%     Tells the script you want nonbolded text at a fontsize of 10 with verdana as a font and
%     the numberformat to be three digits after a decimal point. If you
%     want text you just rewrite the NumberFormat argument as '@'
%
%   SUCCESS = XLSWRITE('c:\matlab\work\myworkbook.xls',A,'A2:C4',params) will write A to
%   the workbook file, myworkbook.xls, and attempt to fit the elements of A into
%   the rectangular worksheet region, A2:C4. On success, SUCCESS will contain true,
%   while on failure, SUCCESS will contain false.
%   
...
%   Hacked a little bit to offer better formatting
%   in excel by Chand.
%
%==============================================================================
% Set default values.
...
try
    ...
    % SET FOR THE SELECTION CERTAIN PARAMETERS
    try
    if ~isempty(params)
        Excel.selection.Font.Bold = params(1).Bold;
        Excel.selection.Font.Name = params(1).FontName;
        Excel.selection.Font.Size = params(1).FontSize;
        Excel.selection.Font.ColorIndex = params(1).ColorIndex;
        Excel.selection.NumberFormat = params(1).NumberFormat;
    end
    catch
    end
