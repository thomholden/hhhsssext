function [out]=sarmap2(long,lat,xinp,yinp,W,varargin)
% PURPOSE: adjust a spatial autoregressive model on a subregion selected on the map
%--------------------------------------------------------------
% USAGE: out = sarmap(lat,long,xinp,yinp,W,carte,label,symbol)
%   where: 
...
for i=1:nbev
   if vnames_flag == 1
    Hbhatglobp(i)=uicontrol('Style','text','Units','Normalized','Position',[0.55,0.88-(i-1)*0.02,0.070,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',strjust(vnames(i+1,1:15),'left'),'enable','inactive');
...
Hrhoglob=uicontrol('Style','text','Units','Normalized','Position',[0.6,fyb-0.04,0.12,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',['Rho=',strjust(num2str(roundoff(results.rho,4)),'left')],'enable','inactive','enable','inactive');
Hsigeglob=uicontrol('Style','text','Units','Normalized','Position',[0.6,fyb-0.08,0.12,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',['Sige=',strjust(num2str(roundoff(results.sige,4)))],'enable','inactive','enable','inactive');
Hrbarglob=uicontrol('Style','text','Units','Normalized','Position',[0.6,fyb-0.12,0.12,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',['Rbar-squared=',strjust(num2str(roundoff(results.rsqr,2)))],'enable','inactive');
Hloglikglob=uicontrol('Style','text','Units','Normalized','Position',[0.6,fyb-0.16,0.16,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',['Log-likelihood=',strjust(num2str(roundoff(results.lik,4)))],'enable','inactive');
Hnobsglob=uicontrol('Style','text','Units','Normalized','Position',[0.6,fyb-0.20,0.12,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String',['Nobservations=',num2str(results.nobs)],'enable','inactive');
...
Hlogliksel=uicontrol('Style','text','Units','Normalized','Position',[0.8,fyb-0.16,0.16,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String','Log-likelihood=','enable','inactive');
Hnobssel=uicontrol('Style','text','Units','Normalized','Position',[0.8,fyb-0.20,0.12,0.026],'Backgroundcolor',[0.8,0.8,0.8],'String','Nobservations=','enable','inactive');
...
while BUTTON~=2 & BUTTON~=113 % Stop when the user push the middle button or press 'q'
    ...
    if ~isequal(oldobs,obs) & ~isempty(Iselect)
        results=sar(yinp(Iselect),xinp(Iselect,:),W(Iselect,Iselect));
        for i=1:nbev
