...
%SLS2  Markov Chain Monte Carlo sampling using Slice Sampling
%
%  Description
...
%      or not. See SLS_OPT for details.
%
...
for i = 1-nomit:1:nsamples
  ...
  for j = 1:nparams
    ...
    if isinf(y)
      ...
      if overrelaxation(j)
        [x_new, y_new, rej_step, rej_old] = bisection(f,y,x_new,L,R,w,a,rej_step,j,varargin{:});        
      else
        [x_new, y_new] = shrinkage(f,y,x_new,w,L,R,method,j,maxiter,umodal,varargin{:});
      end % if overrelaxation
      if umodal
        w(j) = (w(j)*ind_umodal + abs(x_0-x_new))/(ind_umodal+1);
      end % if umodal
    end % if isinf(y)
  end % j:nparams
...
function [x_new, y_new, rej, rej_old] = bisection(f,y,x_0,L,R,w,a,rej,j,varargin);
%function [x_new, y_new, rej, rej_old] = bisection(f,y,x_0,L,R,w,a,rej,j,varargin);
%
...
if um % if the user defines the distribution to be unimodal
  ...
  if y < -feval(f,L,varargin{:}) && Ao == 0
    A = 1;
  else
    A = 0;
  end
  if y < -feval(f,R,varargin{:}) && Bo == 0
    B = 1;
  else
    B = 0;
  end
