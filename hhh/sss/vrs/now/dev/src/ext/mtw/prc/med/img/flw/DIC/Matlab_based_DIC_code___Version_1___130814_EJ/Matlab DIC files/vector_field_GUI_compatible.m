function vector_field_GUI_compatible(figpos,quiver_param,N_images,skip,dispx,dispy,scale)
...
for i = 1:skip:N_images
    ...
    axis equal
    title(['Vectorfield of displacements',sprintf(' (Current image #: %1g)',i)],'fontsize',16,'fontweight','bold')
    xlabel(['X ',unit],'fontsize',16)
    ylabel(['Y ',unit],'fontsize',16)
    set(gca,'fontsize',12)
    try
        waitforbuttonpress
    catch
