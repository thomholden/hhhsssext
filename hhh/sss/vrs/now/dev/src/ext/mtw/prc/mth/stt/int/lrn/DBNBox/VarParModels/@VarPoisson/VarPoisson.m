function [obsmodel]=VarPoisson(varargin);
% [obsmodel]=VarPoisson(K,X,options);
% contructor for variational Poisson observation models 
%
...
% $$$   Gamma_alpha     Posterior Shape
% $$$   Gamma_beta      Posterior Scale
% $$$   prior           prior to parameters above
% $$$      Gamma_alpha     Prior count per interval (shape)
% $$$      Gamma_beta      Prior Mean rate (scale)
% $$$ 
% $$$ 
% $$$ Methods
% $$$ 
...
ClassName='VarPoisson';
% default state space dimension
defK=2;
% default hmm model structure
defobsmodelstruct=struct('Gamma_alpha',[],'Gamma_beta',[],...
      'prior',struct('Gamma_alpha',[],'Gamma_beta',[]),'options',[]);
