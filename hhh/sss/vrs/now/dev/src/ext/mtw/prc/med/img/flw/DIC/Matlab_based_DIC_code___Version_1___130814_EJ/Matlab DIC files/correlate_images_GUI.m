...
% Last Modified by GUIDE v2.5 01-Jul-2013 13:37:29
...
if corr_reduced_selection==2 || corr_reduced_selection==3 
   %Yes                      OR Yes - use previous iteration    
    ...
    if mod(subset_reduced,2) ~= 0 %The subset is NOT even
        w = warndlg('The subset size must be even!','Even subset');
        waitfor(w);
        return;
    end
    ...
    save('disp_reduced_data','dispx_reduced','dispy_reduced');
...
if mod(subset,2) ~= 0 %The subset is NOT even
    w = warndlg('The subset size must be even!','Even subset');
    waitfor(w);
    return;
end
...
if correlate_full_selection==2 %Yes
    ...
    %the full images.
    search_zone = 2*ones(N_pts,N_images_correlated); 
    ...
    elseif use_reduced_selection==2 %Yes
        ...
        [initialx,initialy] = grid_correlation(gridx_reduced,gridy_reduced,gridx,gridy,...
            grid_setup,dispx_reduced,dispy_reduced,reduction_prev,N_images_correlated);
    ...
    [validx,validy] = automate_image_GUI_compatible...
        (filenamelist,reduction_full,gridx,gridy,ref_image,subset,search_zone,...
        N_images_correlated,N_pts,initialx,initialy,N_threads,loop_type);

    t3 = clock;


    save('grid_data','gridx','gridy');
    save('valid_data','validx','validy');
