...
classdef hestonmodelbuilder < modelbuilder
    % hestonmodelbuilder initializes a hestonmodel
    %   first a heston model is initalized 
    %   second the parameters are initialized wrt input data
    
    properties
        pmodel; % this is a Heston model
    end
    
    methods
        function m = hestonmodelbuilder(params)
        if (nargin == 0 || isempty(params))
            ...
            m.pmodel = hestonmodel(params);
        end
    end
    
    methods (Static)
        function y = buildparam(params)
            y.pmodel = hestonmodel(params);
        end
