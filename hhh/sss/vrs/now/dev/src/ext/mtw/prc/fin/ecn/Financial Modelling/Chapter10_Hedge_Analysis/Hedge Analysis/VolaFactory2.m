...
function VolaParas = VolaFactory2(model,data,levyData)

    if strcmp(model.ID,'BlackScholes')
        VolaParas = @(i,strike,spot,mat)volextrap(data,i,strike,spot,mat);
...
function volas = volextrap(data,i,strike,spot,mat)
    ...
    volas = interp2(data.volaMaturities,data.volaStrikes,data.volaValues(:,:,i),mat,moneyness,'spline'); 
