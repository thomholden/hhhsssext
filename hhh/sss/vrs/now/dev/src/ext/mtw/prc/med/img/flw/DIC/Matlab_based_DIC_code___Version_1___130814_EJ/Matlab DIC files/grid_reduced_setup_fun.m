...
function [gridx_reduced,gridy_reduced,grid_setup_reduced,corr_setup_reduced,...
    dispx_reduced,dispy_reduced] = grid_reduced_setup_fun(...
...
%Search zone for reduced images - set to 3 and should not change.
...
if reduced_grid_selection==1 && ~isempty(grid_setup_reduced) %No
    ...
    search_zone_reduced = def_search_zone*ones(N_pts_reduced,N_images_correlated);
...
[validx_reduced,validy_reduced] = automate_image_GUI_compatible...
    ...
    initialx_reduced,initialy_reduced,N_threads,loop_type);
...
corr_setup_reduced.def_search_zone = def_search_zone;
