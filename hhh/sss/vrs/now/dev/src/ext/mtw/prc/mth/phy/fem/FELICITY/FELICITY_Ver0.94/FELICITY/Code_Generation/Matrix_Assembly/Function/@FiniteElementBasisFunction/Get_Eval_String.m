...
if ~CONST
    ...
    if ~strcmp(input_str(1:len_var_str),var_str)
    ...
    if strcmp(obj.Type,'Test')
        ...
        %basis_func_STR  = 'row_basis_func.';
    elseif strcmp(obj.Type,'Trial')
        basis_index_STR = '[j]'; % col index
        %basis_func_STR  = 'col_basis_func.';
    else
    ...
    if ~isempty(regexp(input_str,[var_str, MATCH_STR.grad], 'once'))
        ...
        Replacement_str = Func_Grad_Replacement(obj.FuncTrans,Vector_Comp,Grad_Comp,basis_func_STR,basis_index_STR);
    ...
    elseif ~isempty(regexp(input_str,[var_str, MATCH_STR.div], 'once'))
        ...
        Replacement_str = Func_Div_Replacement(obj.FuncTrans,Vector_Comp,basis_func_STR,basis_index_STR);
        
    % check if it is the arc-length derivative along a curve
    elseif ~isempty(regexp(input_str,[var_str, MATCH_STR.ds], 'once'))
        
        obj.Opt.d_ds = true;
        if obj.Elem.Top_Dim > 1
            error('Cannot use arc-length derivative in topological dimensions greater than 1.');
        end
        Replacement_str = Func_d_ds_Replacement(obj.FuncTrans,Vector_Comp,basis_func_STR,basis_index_STR);
...
function STR = Func_Grad_Replacement(FuncTrans,Vector_Comp,Grad_Comp,basis_func_STR,basis_index_STR)

CPP_Name      = FuncTrans.Output_CPP_Var_Name('Grad');
CPP_Name_w_BF = [basis_func_STR, CPP_Name];

STR_begin = [CPP_Name_w_BF, basis_index_STR, '[qp]'];
Eval_Ext = FuncTrans.Output_CPP_Eval_Extension('Grad',[Vector_Comp, Grad_Comp]);
STR = [STR_begin, Eval_Ext];

end

function STR = Func_Div_Replacement(FuncTrans,Vector_Comp,basis_func_STR,basis_index_STR)

CPP_Name        = FuncTrans.Output_CPP_Var_Name('Div');
CPP_Name_w_BF   = [basis_func_STR, CPP_Name];

STR_begin = [CPP_Name_w_BF, basis_index_STR, '[qp]'];
Eval_Ext = FuncTrans.Output_CPP_Eval_Extension('Div',Vector_Comp);
STR = [STR_begin, Eval_Ext];

end

function STR = Func_d_ds_Replacement(FuncTrans,Vector_Comp,basis_func_STR,basis_index_STR)

CPP_Name        = FuncTrans.Output_CPP_Var_Name('d_ds');
CPP_Name_w_BF   = [basis_func_STR, CPP_Name];

STR_begin = [CPP_Name_w_BF, basis_index_STR, '[qp]'];
Eval_Ext = FuncTrans.Output_CPP_Eval_Extension('d_ds',Vector_Comp);
STR = [STR_begin, Eval_Ext];
