...
fprintf(fid, [TAB2, 'unsigned int DoE_Elem_Index = ', CPP_DOM.Interp_Data, '.Cell_Index[DoE_Pt] - 1',...
                    '; // need to offset for C-style indexing', ENDL]);
%fprintf(fid, [TAB2, 'mexPrintf("DoE_Elem_Index = %%d.\\n",', 'DoE_Elem_Index + 1', ');', ENDL]);
...
for gi = 1:length(GeomFunc)
    ...
    fprintf(fid, [TAB2, CPP_Domain.Identifier_Name, '.Read_Embed_Data(DoE_Elem_Index);', ENDL]);
...
for gi = 1:length(GeomFunc)
    ...
    fprintf(fid, [TAB2, '// copy local interpolation coordinates', ENDL]);
    Mesh_Local_Coord_str = [Mesh_Name_str, '.local_coord'];
    fprintf(fid, [TAB2, CPP_DOM.Interp_Data, '.Copy_Local_X(DoE_Pt,', Mesh_Local_Coord_str, ');', ENDL]);
...
if ~isempty(Basis_Func)
    %%%%%%%
    % output text-lines
    fprintf(fid, [TAB2, '// perform pre-computations with FE basis functions', ENDL]);
    fprintf(fid, [TAB2, '// NOTE: this must come before the external FE coefficient functions', ENDL]);
    for index = 1:length(Basis_Func)
        BF_Name_str = Basis_Func{index}.CPP_Var;
        fprintf(fid, [TAB2, '// copy local interpolation coordinates', ENDL]);
        BF_Local_Coord_str = [BF_Name_str, '.local_coord'];
        fprintf(fid, [TAB2, CPP_DOM.Interp_Data, '.Copy_Local_X(DoE_Pt,', BF_Local_Coord_str, ');', ENDL]);
        Func_Call_str = [BF_Name_str, '.Transform_Basis_Functions();'];
        fprintf(fid, [TAB2, Func_Call_str, ENDL]);
    end
    fprintf(fid, ['', ENDL]);
    %%%%%%%
end
...
fprintf(fid, ['', ENDL]);
fprintf(fid, [TAB2, '// loop through the desired FEM interpolations', ENDL]);
...
for ind = 1:NUM_INTERP
    ...
    fprintf(fid, [TAB2, CPP.Var_Name, '->Eval_All_Interpolations(DoE_Pt);', ENDL]);
end
fprintf(fid, [TAB2, '}', ENDL]);
