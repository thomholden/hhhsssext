...
%
% Function EDFT produce discrete N-point Fourier transform F and amplitude spectrum S
% of the data vector X. Data X may contain NaN (Not-a-Number).
%
% SYNTAX
% [F,S,Stopit]=edft(X,N) for N>length(X) calculate F and S iteratively 
% 	(see an ALGORITHM below). If data X do not contain NaN and N<=length(X)
%	or N is not specified, EDFT return the same results as fast Fourier 
%	transform: F=fft(X,N) or F=fft(X) and S=F/N.
% [F,S,Stopit]=edft(X,N,I) performs edft(X,N) with limit I for maximum number of iterations.
%	Default value for I is set by parameter 'Miteration', that is, 
%	edft(X,N)=edft(X,N,Miteration). 
%	To complete iteration process faster, the value for 'Miteration' should be decreased.
% [F,S,Stopit]=edft(X,N,I,W) execute edft(X,N,I) with initial conditions defined by weight
%	vector W. Default values for W are ones(size(F)). W must have at least 
%	length(X) nonzero elements.
% Stopit is an informative (optional) output parameter. The first row of Stopit showing
%	the number of performed iteration, the second row indicate breaking of iteration
%	reason and may have the following values: 
%	0 - Maximum number of iteration performed. If length(X)<=N, only one EDFT 
%	    iteration is performed (I=1).  
%	1 - Sum of outputs division sum(F./S) is not equal to length(X)*N within Relative 
%	    deviation 'Rdeviat'. The calculations were interrupted  because of results could
%	    be inaccurate. If this occur in the first iteration, then outputs F and S are zeros.
%	2 - Relative threshold 'Rthresh' reached. To complete iteration process faster,
...
%	W - (optional) weight vector W. If not specified, W = ones(1,N) is used
%	   for the first iteration.
%	E - Fourier transform basis matrix: E=exp(-i*2*pi*(0:length(X)-1)'*(0:N-1)/N);
%	   If part of unknown data in X are replaced by NaN then the time vector
%	   (0:length(X)-1) is changed to exclude time moments where NaN inserted.
...
% A special case: If length(X) is equal to N, the EDFT output do not depend on selected
%	   weight vector W and is calculated in non-iterative way.
% FEATURES
%	1. EDFT output F is the N-point Fourier transform of data X.
%	   The Power Spectral Density (PSD) function can be calculated by the following
%	   formula: abs(F).^2/(N*T), T - sampling period.
%	2. EDFT can extrapolate input data X to length N. That is, if apply EDFT for
%	   N>length(X), get the results: F=edft(X,N)=edft(Y)=fft(Y); Y=ifft(F),  where
%	  Y is input X plus non-zero forward and backward extrapolation of X to length N.  
%	3. EDFT output S estimate amplitudes and phases of sinusoidal components in
%	   input data X.
%	4. EDFT can increase frequency resolution N/length(X) times. Division of outputs
%	   1/(T*F./S) demonstrate the frequency resolution of EDFT. The following is true
%	   for any EDFT iteration:
%		0<F./S<=N,
%		sum(F./S)=N*length(X).
%	5. EDFT input data X may contain NaN. NaN indicate unavailable data or missing
%	   samples or data segments in X. EDFT Outputs F and S are calculated by
%	   applying slower algorithm then in case of X without NaN.
...
%		incomplete data", Automatic control and computer sciences,
...
%	INVR(t,t)=pinv(R(t,t));		% Pseudoinverse if R is nearly singular	
	ER(1)=trace(INVR);
	for k=1:K-1
