...
NN -  Creates forecasts of a time series on t+1 using nearest neighbour
  ...
  FERN�NDEZ-RODR�GUEZ, F., SOSVILLA-RIVERO, S., GARC�A-ARTILES, M. An
...
function [OutSample_For,InSample_For,InSample_Res]=nn(x,d,m,k,method,n) %#codegen
...
InSample_For = zeros(length(x)-d,1);
for v = 0:(length(x)-d-1)
    InSample_For(v+1) = nn_core(x(1:d+v),m,k,method);
end
InSample_Res=x(d+1:length(x))-InSample_For;
OutSample_For = zeros(n,1);
if n ~= 0
    x2 = x;
    for z = 1:n
        OutSample_For(z) = nn_core(x2,m,k,method);
