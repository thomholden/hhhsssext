...
%    F=nedft(X,tk,fn)
%	Function NEDFT returns discrete Fourier transform F of input sequence X sampled
%	at arbitrary selected time moments tk: X(tk) > F(fn), where frequencies fn, in 
%	general, may be selected arbitrary. If fn is less than X, inputs X and tk will be
%	truncated.
...
%	'Miteration', that is, nedft(X,tk,fn)=nedft(X,tk,fn,Miteration). To complete
%	iteration process faster, the value for 'Miteration' should be decreased.
...
%  Stopit is an informative output parameter. The first row of Stopit showing the number of 
...
%	deviation 'Rdeviat'. The calculations are interrupted because of results could be
%	inaccurate. If this occur in the first NEDFT iteration, then outputs F and S are zeros.
...
%	W- (optional) weight vector W. If not specified, W = ones(1,size(fn)) used for 
%	    the first iteration.
...
%	where tk=[0,1,...,K-1]*T and T is sampling period. Nonuniform sampling can be
%	realized in many different ways, like as:
...
%	- uniform sampling with jitter: tk=([0,1,...,K-1] + jitter*rand(1,K))*Ts, where value
%	for jitter is selected in range [0...1[ and Ts is the mean sampling period;   
...
%	2. Frequencies for fn can be selected arbitrary. This mean, that user can choose not
%	only the length of NEDFT (number of frequencies in fn), but also the way how to
%	distribute frequencies along the frequency axis. On other hand, to get adequate
%	sequence X representation, frequencies fn should be selected to cover overall range,
%	where the input sequence X spectrum is supposed to be found, otherwise, in result
%	of NEDFT, all components having spectra outside fn will be incorporated.
...
%	combine different frequency sets in one or just add individual frequencies of
%	interest to fn, e.g, fn=[fn1 fn2 f1 f2], where fn1 and fn2 are different frequency
%	sets, f1,f2 - specific frequencies. NEDFT outputs will be calculated accordingly-
...
%	in sequence X(tk).
%	4. NEDFT can increase frequency resolution length(fn)/length(X) times. Division of
%	outputs 1/(Ts*(F./S)) demonstrate the frequency resolution of NEDFT.
...
% Reference: 	V.Liepin'sh, "A spectral estimation method of nonuniformly sampled 
%		band-limited signals. Automatic Control and Computer Sciences,
%		Vol.28, No.2, pp.66-77, 1994.
...
% Calculate the correlation matrix R by using vectorized form and RE=R\E (an alternative).
%	R=E*diag(W(l,:)/N)*E';
%     	RE=R\E;
