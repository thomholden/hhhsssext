...
%Function to clear a selected consequent goal from list
function clearOneConRule()
...
switch response 
   %If yes button is pressed
	case 'Yes'
      %If there is only one rule set variable to empty
      if size(rules,2) == 1
         new_rules = [];
      %Otherwise if this part is reached there must be multiple items in list
      else   
      	j = 1;
      	%Copy all rules over, excluding selected rule for deleting
