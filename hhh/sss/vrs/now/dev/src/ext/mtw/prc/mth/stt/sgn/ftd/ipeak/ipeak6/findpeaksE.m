...
% function
% P=findpeaks(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smootht
...
% C. O'Haver, 1995.  Version 5.2, Last revised February, 2014
    ...
    if sign(d(j)) > sign (d(j+1)), % Detects zero-crossing
        if d(j)-d(j+1) > SlopeThreshold*y(j), % if slope of derivative is larger than SlopeThreshold
            if y(j) > AmpTest,  % if height of peak is larger than AmpThreshold
                ...
                Error=100.*abs(std(residual)./PeakY);
