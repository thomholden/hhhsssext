...
classdef aae < objectivefuncbase
    % absolute percentage error
    properties
        dataMarket = [];    % market data   (quoted prices)
        weights = [];       % weights for model prices
        NOpt  = [];         % Number of Calibration Instruments
    end
    
    methods
        function y = val(OBJ, x)
            % absolute percentage error
            y = 1/(OBJ.NOpt) * sum(sum(abs(OBJ.weights.*(OBJ.dataMarket-x))));
        end        
    end
    
    methods
        function OBJ = aae(dataMarket_,weights_)
