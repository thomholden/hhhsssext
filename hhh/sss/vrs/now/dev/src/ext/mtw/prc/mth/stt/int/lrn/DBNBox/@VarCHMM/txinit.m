...
%
  ...
  for c=1:chmm.NChains,
    ...
    if strcmp(txmodelname,'ConstrTxModels')
      selfndx=find((chmm.LagOpSpec{c}(2,:)==c) & ...
	       (chmm.LagOpSpec{c}(1,:)==-1));
    end

    % initialise state transition model
    switch txmodelname
     case 'TxModels'
      txmodel=VarTxModels(Kc,txmodeloptions);
     case 'ConstrTxModels'
      txmodel=VarConstrTxModels(Kc,selfndx,txmodeloptions);
     otherwise
