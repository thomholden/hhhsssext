...
visible = getarg(varargin,'visible',true);
if iscell(visible), visible=visible{:}; end
clines = getarg(varargin,'captionLines',2);
if iscell(clines), clines = clines{:}; end
scale = getarg(varargin,'scale',false);
if iscell(scale), scale = scale{:}; end
captionFunc = getarg(varargin,'captionFunc','');
if iscell(captionFunc), captionFunc = captionFunc{:}; end


    %if a width is specified, all images are given this width. no other caling
    %is performed.
