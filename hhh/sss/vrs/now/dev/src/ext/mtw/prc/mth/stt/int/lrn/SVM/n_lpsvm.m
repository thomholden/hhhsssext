...
%===========================================================================
% Usage: [w, gamma,trainCorr,testCorr,time,nu,mu]=lpsvm(A,d,rr,k,nu,mu,output,delta);
%
% A and d are both required, everything else has a default
% An example: [w gamma train test time nu mu] = n_lpsvm(A,d,.5,10);
%
% Input parameters:
%    A: Data points
%    d: 1's or -1's
...
%       mu:             estimated value (or specified value) of mu
%==========================================================================
...
tic;
[w,gamma,iter] = core(kA,d,nu,delta);
...
tic;
[w, gamma,iter] = core(kCtrain,dtrain,nu,delta);
thisToc = toc;
...
%parameters
epsi=10^(-3);alpha=10^(3);tol=10^(-3);maxiter=50;
[m,n]=size(A);
en=ones(n,1);
em=ones(m,1);
