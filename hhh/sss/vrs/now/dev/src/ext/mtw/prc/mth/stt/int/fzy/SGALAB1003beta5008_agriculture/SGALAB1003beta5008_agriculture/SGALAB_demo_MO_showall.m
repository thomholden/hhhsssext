...
% ====================================================================================================
%File description:
%        To show all MO methods in SGALAB on same fitness value, and the
%        results can be shown in one figure
%Input(1):
%            options[10]:
...
%
%Usage:
%  We can run SGALAB by key-in "SGALAB_DEMO_MO_showall" in Matlab command
%  window
%===================================================================================================
...
%14-Dec-2006 Chen Yi update 1003            leo.chen.yi@gmail.com  Glasgow
%20-Mar-2007 Chen Yi add options(8)         leo.chen.yi@gmail.com  Glasgow
%15-Oct-2009 Chen Yi add n points crossover leo.chen.yi@gmail.com  Glasgow
...
[ maxfitness ,...
    ...
    best_decimal_space ,...
    best_coding_space ,...
    error_status ]= SGA__entry_MO_showall...
    ( options,...
    min_confines ,...
    max_confines ,...
...
% fprintf( fid14 , '%f\n' , maxfitness);
save('OUTPUT_maxfitness.txt', '-struct', 'maxfitness','-ASCII');

%fprintf( fid9, '\n the min value of fitness function:\n');
% fprintf( fid15 , '%f\n' ,minfitness);
save('OUTPUT_minfitness.txt', '-struct', 'minfitness','-ASCII');

%fprintf(fid10,'\n the mean value of fitness function:\n');
% fprintf(fid16,'%f\n',meanfitness);
save('OUTPUT_meanfitness.txt', '-struct', 'meanfitness','-ASCII');

%fprintf( fid11,'\nthe best decimal space(x1 x2 x3...):\n');
% fprintf( fid17,'%f\n',best_decimal_space );
save('OUTPUT_best_result_space.txt', '-struct', 'best_decimal_space','-ASCII');

% fprintf( fid18 , '%f\n' , best_coding_space );
save('OUTPUT_best_coding_space.txt', '-struct', 'best_coding_space','-ASCII');
%fprintf( fid12, '\nthe generation number when end GAs:\n' );

% fprintf( fid19, '%f\n' , now_generation );
save('OUTPUT_now_generation.txt', '-struct', 'now_generation','-ASCII');

% fprintf( fid20, '%f\n' , now_probability_crossover );
save('OUTPUT_now_probability_crossover.txt', '-struct', 'now_probability_crossover','-ASCII');
