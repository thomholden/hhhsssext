function dataout = varnamedlg(varargin)
% This is a modification of the MATLAB function listdlg (for the UI)
% to suit the input of a string parameter suitable as a variable name
%
% Example:
    ...
    switch lower(varargin{i})
        ...
        case 'inputdefaults'
            ...
            for jdx = 1:length(inputdefaults)
                if ~isa(inputdefaults{jdx},'char') || (numel(inputdefaults{jdx}) ~= length(inputdefaults{jdx}))
                    error('''InputDefaults'' property must be a cell of character strings.');
                end
            end
        case 'uh'
            uh = varargin{i+1};
...
boxsize = [200 length(inputstrings)*(uh+fus)];
...
if isappdata(0,'VarNameDialogAppData')
    ad = getappdata(0,'VarNameDialogAppData');
    dataout = ad.varname;
    rmappdata(0,'VarNameDialogAppData')
else
    % figure was deleted
    dataout = [];
end
...
% This will actually check that all inputs are capable of being variable
% names although we'll generally only use it asking for 1 varname
heb = findall(gcbf,'Tag','editboxes');
lid = length(heb);
ad.varname = cell(1,lid);
vars =  evalin('base','who'); % all data in the workspace
for idx = 1:length(heb)
    thisstr = get(heb(idx),'String');
    try
        % create a variabe with this name
        assignin('caller',thisstr,1);
    catch
        str = {[thisstr,' is not a valid variable name.',...
                'Please change it and try again.']};
        errordlg(str,'Variable Name Query Error','modal');
        return
    end
    
    % Now check if a variable with this name already exists
    if any(strcmp(vars,thisstr))
        str = {['A variable with the name ',thisstr,...
                ' already exits.  Overwrite it?']};
        bname = questdlg(str,'Variable Name Query','Yes','No','No');
        switch bname
            case 'Yes'
                ad.varname{idx} = thisstr;
            case 'No'
                return
        end
    else
...
setappdata(0,'VarNameDialogAppData',ad)
delete(gcbf);
