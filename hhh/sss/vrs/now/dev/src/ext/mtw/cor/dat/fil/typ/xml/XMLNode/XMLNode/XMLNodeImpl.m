classdef XMLNodeImpl < handle
    ...
    methods
        ...
        %   >> n = XMLNode(myNode);
        function obj = XMLNodeImpl(node,xpath)
            ...
            %and try and read it
            if ischar(node)
                %See if the input looks like a URL request
                isHTTP = any(strfind(node,'http://')) || ...
                    any(strfind(node,'https://'));
                
                if isHTTP

                    %Read URL
                    urlString = urlread(node);
                    javaString = java.lang.String(urlString);

                    %Load document using namespace aware Java factory
                    documentFactory = DocumentBuilderFactory.newInstance();
                    documentFactory.setNamespaceAware(true)
                    documentBuilder = documentFactory.newDocumentBuilder();
                    bais = ByteArrayInputStream(javaString.getBytes());
                    obj.node = documentBuilder.parse(bais);
                    ...
                    bais = ByteArrayInputStream(javaString.getBytes());
                    byteReader = InputStreamReader(bais,obj.node.getEncoding);
                    reader = inputFactory.createXMLEventReader(byteReader);
                ...
                if ~exist('xpath','var') || isempty(xpath)
                    xpathFactory = XPathFactory.newInstance;
                    obj.xpath = xpathFactory.newXPath;
    ...
    methods (Access=private)
        ...
        % Sometimes, a user might enter something like
        % >> b = XMLNode('test.xml')
        % >> c = b('//PLANTS');
        % >> d = c('//NAME');
        % On that third command, c is actually an array of XMLNodes, not
        % just a single one. So, in these cases, we'll assume that the user
        % wants to apply the XPath expression to each of the nodes and then
        % concatenate the results.
        function res = evaluateXPathExpression(obj,xpathString)
            res = [];
            for i = 1:numel(obj)
                individualRes = obj(i).doEvaluateXPathExpression(xpathString);
                if ~isempty(individualRes)
                    try
                        res = [res,individualRes]; %#ok
                    catch MExc
                        error('XMLNode:CannotConcatenateResult','Unable to create result from XPath');
                    end
                end
            end
        end
        ...
        function res = doEvaluateXPathExpression(obj,xpathString)
            import javax.xml.xpath.*;
            ...
            %TODO: when is this an ArrayList?  Things would be simpler if
            %we knew that this method only returned XMLNodes, so I'm
            %commenting the old behavior out. Please leave a comment if you
            %think this is wrong.
            %Old comment:
            ...
            if ~exist('res','var')
                ...
                if any(methodNameMatch)
                    ...
                    %I don't think we need this warning anymore. It's just
                    %kind of annoying.
