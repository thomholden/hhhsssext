...
for i = max(M,N) : N : length(s)-N
    ...
    try
        ...
        h = 0; % test is outside tables of adftest
    end
    if h ~= 0
        ...
        % The strategy:
        % 1. Compute residuals over next N periods
        ...
        % series by 1 share and long the second series by a scaled number
        % of shares.  If the residuals are large and negative, do the
        % opposite.
