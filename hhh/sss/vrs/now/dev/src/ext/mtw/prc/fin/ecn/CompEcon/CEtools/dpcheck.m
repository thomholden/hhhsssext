...
if isempty(ind1)
  ...
  err=max(abs(df(ind1)-Jf(ind1)));
end
ind2=find(~isinf(d2f));
if isempty(ind2)
  err=[err NaN];
else
  err=[err max(abs(d2f(ind2)-Hf(ind2)))];
end
ind3=find(~isinf(dg));
if isempty(ind3)
  err=[err NaN];
else
  err=[err max(abs(dg(ind3)-Jg(ind3)))];
end
ind4=find(~isinf(d2g));
if isempty(ind4)
  err=[err NaN];
else
  err=[err max(abs(d2g(ind4)-Hg(ind4)))];
end
...
if isempty(ind5)
  ...
  err=[err max(abs(dsf(ind5)-Jf(ind5)))];
end
ind6=find(~isinf(dsg));
if isempty(ind6)
  err=[err NaN];
else
  err=[err max(abs(dsg(ind6)-Jg(ind6)))];
end
end
