...
fprintf('name                 time    flops       norm conv      x\n');
for i=1:5
   tic
   flops(0)
   switch i
   case 1
      name = 'Newton             ';
      optget('newton','maxsteps',0);
      [x,f] = newton('fslv08b',xinit);
      err = norm(f);
   case 2
      name = 'Newton safeguarded ';
      optget('newton','maxsteps',20);
      [x,f] = newton('fslv08b',xinit);
      err = norm(f);
   case 3
      name = 'Broyden            ';
  ...
  fprintf('%s%6.2f %8i   %8.1e   %2i %6.2f\n',name,[toc flops err (err<1.e-7) x])
end
