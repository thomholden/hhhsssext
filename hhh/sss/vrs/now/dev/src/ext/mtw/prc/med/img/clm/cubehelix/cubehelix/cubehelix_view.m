...
% and two colorbars show both the RGB colors and the equivalent grayscale.
%
% Syntax:
%  cubehelix_view
...
% the grayscale colormap is not strictly monotonic increasing/decreasing.
%
...
%
% See also CUBEHELIX CUBEHELIX_FIND BREWERMAP RGBPLOT COLORMAP COLORMAPEDITOR COLORBAR UICONTROL ADDLISTENER
%
% ### Input Arguments ###
%
% Inputs (*=default):
...
switch nargin
    ...
    case 2
        assert(numel(start)==4&&isvector(start),'First input can be a vector of four parameters.')
        assert(numel(rots)==2,'Second input can be a vector of the end brightness levels.')
        chvUpDt(start(1),start(2),start(3),start(4),rots(1),rots(2));
    case 4
        chvUpDt(start,rots,hue,gamma,0,1);
    case 5
        assert(numel(rng)==2,'Fifth input can be a vector of the end brightness levels.')
...
while ishghandle(tgh)&&get(tgh,'Value')
    ...
    isar = prms==ziel;
    ziel(isar) = round(100*cellfun(@(fn)fn(),randfn(isar)))/100;
