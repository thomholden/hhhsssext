function [chmm]=MapCHMM(varargin);
% [chmm]=MapCHMM(LagOp,K,X,obsmodel,options);
%        OR
% [chmm]=MapCHMM(LagOp,K,X,obsmodel,options,Y,outlmodel,outloptions);
%
% contructor for maximum aposteriori coupled hidden markov models 
%
 ...
 otherwise
  if isa(varargin{1},ClassName)
    ...
    if isempty(chains),
	  ...
	  error('Parameter ''chains'' must be of type MapHMM');
       end
    end
