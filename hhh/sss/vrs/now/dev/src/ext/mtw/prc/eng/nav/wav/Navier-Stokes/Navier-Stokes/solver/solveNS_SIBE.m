function [omega psi u v stat X Y f omega0 B0] = solveNS_SIBE(N,f,gleft, ...
    gtop,gright,gbottom,hleft,htop,hright,hbottom,u0,v0,Dt,nu,maxIter,reltol)
%SOLVENS_SIBE  Solve the unsteady Navier-Stokes-problem on [-1,1]x[-1,1] 
%with semi-implicit backward-Euler temporal discretization and Chebyshev
%spectral collocation in space.
%
...
%            B0     - initial convective term
%
%   See also   SOLVENS_ABBD2, SOLVESTOKES, PL_PROBLEM, PTILDE_PROBLEM, 
...
omega_tilde = omega;
psi_tilde = omega;
% Evaluate the known solution at t=0
X_red = X(2:N,2:N);
Y_red = Y(2:N,2:N);
...
for k = 1:maxIter
    if k==1 % initialial time step
        [~,dery_u] = partialder(u0,grid,D1);
        [derx_v,~] = partialder(v0,grid,D1);
        omega0 = derx_v - dery_u;
        omega{1} = omega0;
        % Evaluate the convective term
        B = convective(u0,v0,omega0,grid,D1);
        B = B(2:N,2:N);
        B0 = B;
        F = SIBE(Dt,f,{B},omega0(2:N,2:N),nu);
        difference = [];
    else
        B = convective(u{k-1},v{k-1},omega{k-1},grid,D1);
        B = B(2:N,2:N);
        F = SIBE(Dt,f,{B},omega{k-1}(2:N,2:N),nu);
    end
