...
tit = 'Implied Volatility - Black-Scholes Model';
...
for j=1:rows    %wrt time
    FFTopt(j,:) = blsprice(S0, K, r, T(j), sigma_base, d);
...
for j=1:rows    %wrt time
    FFTopt_low(j,:) = blsprice(S0, K, r, T(j), sigma_low, d);
    FFTopt_high(j,:) = blsprice(S0, K, r, T(j), sigma_high, d);
