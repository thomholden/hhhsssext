...
%      (1) display all information while the chain advances (energies,
%          rejections in case overrelaxation, overflows when stepping-out
%          or doubling etc.)
%      (2) verbose-mode (display only the overflows)
%    method ('stepping')
%      whether to use stepping-out ('stepping'), doubling ('doubling')
%      or ('minmax') to grow the slice
%    overrelaxation (0)
...
%      (overrelaxation)
%    wsize (1)
%      estimate of the typical slice size w (stepping-out, doubling)
%    mlimit (4)
...
elseif opt.overrelaxation == 1 && opt.method == 'doubling'
  opt.method = 'stepping';
end
