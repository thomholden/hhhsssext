...
classdef PropertyGridField < hgsetget
    ...
    methods
        ...
        function set.DisplayName(self, name)
            if isempty(name)
                self.DisplayName = [];
            else
                validateattributes(name, {'char'}, {'nonempty','row'});
        ...
        function set.Description(self, description)
        ...
        function set.ReadOnly(self, readonly)
    ...
    methods (Access = private)
        ...
        %    hu.bme.aut.www (not a direct child)
            ...
            ix = strmatch(prefix, names);  %#ok<MATCH2> % get names that begin with prefix
            if isempty(ix)
            ...
            names = cellfun(@(name) name(len+1:end), names, ...            % drop leading prefix from names
                'UniformOutput', false);
            filter = cellfun(@(name) ~contains(name, '.'), names);  % property names that become simple after prefix is chopped
