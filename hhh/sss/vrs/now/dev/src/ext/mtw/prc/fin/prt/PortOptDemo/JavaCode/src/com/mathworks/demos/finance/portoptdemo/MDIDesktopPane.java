...
public class MDIDesktopPane extends JDesktopPane {
    ...
    private MDIDesktopManager fManager;
    private Hashtable<String, Rectangle>   fDefaultLayoutHashTable;

    public MDIDesktopPane() {
        fManager=new MDIDesktopManager(this);
        setDesktopManager(fManager);
    ...
    public void defaultLayout()
    {
        ...
        if(fDefaultLayoutHashTable != null)
        {
            for (int i=0; i < allFrames.length; ++i)
            {
                Rectangle rect = (Rectangle)fDefaultLayoutHashTable.get(((JInternalFrame)allFrames[i]).getTitle());
                allFrames[i].setSize(rect.width, rect.height);
                allFrames[i].setLocation(rect.x, rect.y);
            }
        }
    }
    ...
    public void generateDefaultFromCurrentLayout()
    {
        fDefaultLayoutHashTable = new Hashtable<String, Rectangle>();
        java.awt.Component allFrames[] = getAllFrames();

        for(int i=0; i < allFrames.length; ++i)
        {
            Rectangle rect = allFrames[i].getBounds();
            String name = ((JInternalFrame)allFrames[i]).getTitle();
            
            fDefaultLayoutHashTable.put(name, rect);
        }
    }
    ...
    private void checkDesktopSize() {
        if (getParent()!=null&&isVisible()) fManager.resizeDesktop();
    }
}
