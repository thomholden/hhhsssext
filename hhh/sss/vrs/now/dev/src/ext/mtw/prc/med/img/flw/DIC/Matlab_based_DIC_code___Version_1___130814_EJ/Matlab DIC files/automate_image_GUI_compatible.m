...
function [validx,validy]=automate_image_GUI_compatible(filenamelist,...
    reduction,gridx,gridy,ref_image,subset,search_zone,N_images_correlated,...
    N_pts,initialx,initialy,N_threads,loop_type)
...
[validx, validy] = serial(filenamelist,reduction,gridx,gridy,...
    validx,validy,ref_image,subset,search_zone,N_images_correlated,N_pts,...
    initialx,initialy,ppm);

elseif strcmp(loop_type,'parallel') == 1 %Use parallel loops
[validx,validy] = parallel(filenamelist,reduction,gridx,gridy,...
    validx,validy,subset,search_zone,N_images_correlated,N_pts,N_threads,...
    initialx,initialy,ppm);
...
function [validx,validy] = serial(filenamelist,reduction,gridx,gridy,...
    validx,validy,ref_image,subset,search_zone,N_images_correlated,N_pts,...
    initialx,initialy,ppm)
...
for i=1:N_images_correlated %Loop over all the images in the set
    ...
    [validx_i,validy_i] = cpcorr_loop(filenamelist, reduction,... 
        subset,search_zone,base_points,base,N_pts,initialx,initialy,i);
...
function [validx,validy] = parallel(filenamelist,reduction,gridx,gridy,...
    validx,validy,subset,search_zone,N_images_correlated,N_pts,N_threads,...
    initialx,initialy,ppm)
...
function [validx_i,validy_i] = cpcorr_loop(filenamelist, reduction,... 
    subset,search_zone,base_points,base,N_pts,initialx,initialy,i)
...
shifted_points = cpcorr_mod(input_points, base_points, input, base, subset,search_zone(:,i));
