function [rec, net0, rstate] = mlp2r_mc(opt, net, p, t, pp, tt, rec, rstate)
% MLP2R_MC Monte Carlo sampling for model mlpr
%
%   rec = mlp2r_mc(opt, net, p, t, pp, tt, rec, rstate)
%     net     - network
...
%     rstate  - state of the random generator
%
%   Set default options for MLP2R_MC
%    opt=mlp2r_mcopt;
...
% Use function handles? What's the purpose of this, why only w1??
fe=str2fun('mlp2r_e');
fg=str2fun('mlp2r_g');
pw1=net.p.w{1};
...
if nargin < 7 | isempty(rec)
  ...
  ri=size(rec.rejects,1);   % get the numeber of former records
  rec=recappend(rec, ri+opt.nsamples, net0, p, t, pp, tt, 0);
end
...
%Print on the screen output labels
if opt.display
  % Be verbose
  fprintf(' ');
  fprintf('cycle ');
  fprintf('etr    ');    % error train
  if ~isempty(pp)        
    fprintf('etst   ');  % error test
  end
  fprintf('sigma  ');    % variation of residual
  if ~isempty(rec.noiseNus) 
    fprintf('nu  ');     % 
  end
  fprintf(' ');
  if isfield(rec,'inputHiddenHyperNus')
    fprintf('w1nu ', rec.inputHiddenHyperNus(ri)) % 
  end
  fprintf('rej   ');      % rejection rate
  fprintf('\n');
end
...
for k=1:opt.nsamples
  ...
  rejs = 0;
  for l=1:opt.repeat % --------- start repeat ----------------
  ...
  % -----------  Sample inputs with RJMCMC -----------------------------------------
    ...
    if isfield(net,'inputii')
      ...
      net0.p.r=net.p.r;
      if isfield(net.p,'k')
	net0.p.k.a.alpha=net.p.k.a.alpha;
      end
    else
      net0=net;
    end
  ...
  if opt.plot
    ...
    subplot(2,1,1)
    if ~isempty(pp)
      semilogy(1:ri,rec.etr(1:ri,:),1:ri,rec.etst(1:ri,:));
      legend('train','test');
    else
      semilogy(1:ri,rec.etr(1:ri,:));
      legend('train');
    end
    ylabel('rmse');
    axis tight
    line([1 ri],[ebase ebase]);
    subplot(2,1,2)
    semilogy(1:ri,[rec.noiseSigmas(1:ri,:)]);
    drawnow
  end
...
% record for hyperparameters of noise (residual) 
if isfield(net.p.r.p,'s') & isfield(net.p.r.p.s,'p')
  rec.noiseHyper(ri,:)=net.p.r.p.s.a.s(:)';
else
  rec.noiseHyper=[];
end

%record for sigma/Nu of noise distribution
rec.noiseSigmas(ri,:)=net.p.r.a.s(:)';
if isfield(net.p.r.p,'nu')
  rec.noiseNus(ri,:)=net.p.r.a.nu(:)';
else
  rec.noiseNus=[];
end
...
rec.etr(ri,:)=sqrt(mean(mean((t-mlp2fwd(net, p)).^2)))./std(p);
...
if ~isempty(pp)
  ...
  rec.etst(ri,:)=sqrt(mean(mean((tt-mlp2fwd(net, pp)).^2)))./std(pp);
end
