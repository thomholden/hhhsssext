		 ...
		 const int nrhs, const mxArray *prhs[])
{
  ...
  
  {
	...
	/* Evaluates b=C*ty */
	dsymv_(uplo, &m, &done, C, &m, ty, &one, &dzero, b, &one);
	/* evaluates y=K*b */
 	dgemv_(trans, &m2, &m, &done, K, &m, b, &one, &dzero, y, &one);   
	mxFree(b);
      }
	...
	dpptrf_(uplo, &m, C, &info);
	/* Evaluates b <- inv(C)*b  (solves C*x= b to b) */
	dpptrs_(uplo, &m, &one, C, b, &m, &info);
	/* Evaluates y=K*b
	   NOTE: Above in evaluation of K the we get K' */
 	dgemv_(trans, &m2, &m, &done, K, &m2, b, &one, &dzero, y, &one);
	mxFree(C);
	mxFree(b);
      }
      mxFree(K);
	...
	/* Evaluates y=K*b
 	...
 	dspmv_(uplo, &m, &done, K, b, &one, &dzero, y, &one);
	mxFree(K);
	mxFree(b);
      }
