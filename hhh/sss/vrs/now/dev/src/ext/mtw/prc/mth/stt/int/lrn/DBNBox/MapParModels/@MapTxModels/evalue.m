function [avLL,lPri] = evalue (txmodel,Xi,Gamma,T,varargin);
% [logprior] = evalue (txmodel,Xi,Gamma,T);
%
% Evaluates the data and parameter log-likelihood for the state transition model
% 
% INPUT
%
...
avLL=0; 
lPri=dirichlet(Pi(:),txmodel.prior.Dir1d_alpha(:),1);
for l=1:K(1),
  % intial state avLL
  if Pi(l)~=0,
...
for l=1:K(2),
  lPri=[lPri dirichlet(P(:,l),txmodel.prior.Dir_alpha(:,l),1)];
  for k=1:K(1),
    if P(k,l)~=0
