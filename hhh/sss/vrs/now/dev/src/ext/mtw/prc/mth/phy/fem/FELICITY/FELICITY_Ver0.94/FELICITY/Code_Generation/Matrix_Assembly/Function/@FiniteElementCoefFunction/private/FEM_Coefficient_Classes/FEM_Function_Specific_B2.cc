    ...
    if (CHK_Num_Comp != Num_Comp)
        {
        mexPrintf("ERROR: Function Nodal Value List has %d columns; expected %d columns.\n", CHK_Num_Comp, Num_Comp);
        mexPrintf("ERROR: You should check: Name = %s, Type = %s!\n",Name,Type);
        mexErrMsgTxt("ERROR: number of function components must match!");
        }
    if (Temp_Num_Nodes < basis_func->Num_Nodes)
        {
        mexPrintf("ERROR: Function Nodal Value List has %d rows; expected at least %d rows.\n", Temp_Num_Nodes, basis_func->Num_Nodes);
        mexPrintf("ERROR: You should check: Name = %s, Type = %s, \n",Name,Type);
    ...
    if (basis_func->Num_Basis != NB)
        {
        mexPrintf("ERROR: Coefficient Function has %d basis functions,\n", NB);
