function [xyinput] = cpcorr_mod(varargin)
...
[xyinput_in,xybase_in,input,base,subset,search_zone] = ParseInputs(varargin{:});
...
%-------------------------------
%
function [xyinput_in,xybase_in,input,base,subset,search_zone] = ParseInputs(varargin)
...
%EJ New Check:
%Eliminate the check on the input points; instead, move this check to
%within the loop over the control points.  If an input point is out of the
%image, make that point not correlate.
if any(xybase_in(:)<0.5) || any(xybase_in(:,1)>size(base,2)+0.5) || ...
   any(xybase_in(:,2)>size(base,1)+0.5)
    error(message('images:cpcorr:cpPointsMustBeInPixCoord'))
end
