function [txmodel]=VarTxModels(varargin);
% [txmodel]=VarTxModels(K,txmodeloptions,priors);
% contructor for variational observation models 
%
...
% $$$ Methods
% $$$ 
% $$$   hsinit()             initialise observation model
% $$$   hslike()             evaluate data likelihood under model
% $$$   hsupdate()           udpate observation model parameters
% $$$   evalue()             evaluate free energy of observation model

  

ClassName='VarTxModels';
% default state space dimension
defK=2;
% default options
defaultoptions=struct('comppri',1);
% define default P-priors
defhmmprior=struct('Dir1d_alpha',ones(1,defK),'Dir_alpha',ones(defK,defK));
% default hmm model structure
deftxmodelstruct=struct('K',defK,'options',[],'prior',defhmmprior, ...
