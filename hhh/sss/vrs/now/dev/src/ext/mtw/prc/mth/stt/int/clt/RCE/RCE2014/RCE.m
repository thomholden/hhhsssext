function history = RCE(in,k, varargin )
%RCE Rapid Centroid Estimation (RCE) clustering (2014)
%   History = RCE(X, K) partitions the points in the Dim-by-N data matrix X
...
%
%   History = RCE(X, K) returns the summary statistics of the search
%
...
%         9: Spearman's Rho
%         10: Standardized Mahalanobis distance ---
%             uses correlation matrix instead of covariance matrix, more stable
%             since the area of the hyper-ellipsoid is equalized throughout all
%             Mahalanobis voronoi regions. The obvious downside is that clusters of
%             varying sizes are now not permitted.
...
%          'm'        - The fuzzifier parameter as per Bezdek's Fuzzy C-means
%          'nsample'  - a parameter from 0 to 1, denoting the rate of
%                       random sample for each subswarm
%          'calculate_labels' - If true (default) calculate the crisp and fuzzy labels for 
%                               each subswarm. Set this to false to prevent
%                               RCE from reporting the labels
%          '
%
%
%   Example:
%       load iris_dataset
%       X = irisInputs;
%       history = RCE(X, 3, 'Distance','beta','m',1.4, 'Display','text', ...
%                             'swarm',3, 'subsprob',0.03, 'maxiter',100);
%       disp('cluster centroids');
%       history.minima
%       figure('name','scatterplot');
%       [~,I] = min(history.fitness_graph(:,end));
%       gplotmatrix(X',[],history.labels{I}')
%       figure('name','fuzzy membership');
%       imagesc(history.softlabels{I})
%
...
%   $Revision: 1.1.0 $  $Date: 2014/10/21 15:19:00 $
    ...
    pnames = {'distance' 'maxiter' 'display' 'subsprob' 'P' 'swarm' 'fun' 'maxstagnation' 'nsample' 'entropy_target' 'evolution' 'm' 'calculate_labels'};
    dflts =  {'sqeuclidean' 100      'text' 0.03 2 3 'wcdist' 15 1 [] false 2 true};
    ...
    distNames = {'euclidean', 'sqeuclidean', 'cityblock', 'minkowski', 'chebyshev','cosine','correlation','sqcorrelation','spearman','normalized_mahalanobis','log-spectral','KL-divergence','JS-divergence','beta', 'jaccard', 'sorensen-dice','hamming'};
    ...
    tic
    [fit, M, Im, numclust, ni, idx_in,Sigma_inv, it_add] = rceoptimize(in,k,m,maxiter,display,max_stagnation,substitution_probability,nsample,nm,f,distance,minkowksi_p, fit_th,evolution);
    t = toc;
    %% format output
    
    
    if(r.calculate_labels)
        history.softlabels = arrayfun(@(x) dist2memb(distmat(in,M{x},distance,minkowksi_p,Sigma_inv{x}),m),1:length(M),'uniformoutput',false);
        history.labels = cellfun(@(x) argmax(x,1),history.softlabels,'uniformoutput',false);
    end
    ...
    history.numclust = uint32(numclust);
    
    history.ni = ni;
    
    history.rsample = idx_in;
    history.t = t;
    
    history.Sigma_inv = Sigma_inv;
    history.distance = distance;
    history.n_particles = uint32(it_add); 
    clearvars -except history in;
...
function [fit, M, Im, numclust, ni, idx_in,Sigma_inv, k_particles] = rceoptimize(in,K,m_f,maxiter,display,max_stagnation,substitution_probability,nsample,nm,fun,distance,P, fit_th,evolution)
...
% max_stagnation = 15;
    ...
    for it = 1:maxiter
        ...
        for m = 1:nm
            ...
            if(distance == 10)
                ...
                Sigma_tmp = wnancov(in(:,idx_in{m}),U);
            end
            ...
            if(fit_t < fit(m,max(it-1,1)))     
                stagnated(m) = 0;
                ni{m} = ni_m; 
                M = cellfun(@(x,ni) x(:,ni>0),x,ni,'uniformoutput',false);
                ...
                if(evolution)
                   ...
                   if( -mean(U(:).*log2(eps+U(:))) > fit_th(m)); k_particles(m) = k_particles(m) + randi(5); end
                end
            ...
            v{m} = unifrnd(0,1,size(in,1),size(x{m},2)).*(w(it)*v{m} + so + mi + retract);
            x{m} = x{m}+v{m};
    ...
    %            disp('reset');
        ...
        if((display == 2 || display == 3) && min(fit(:,it),[],1) < Ym)
            Ym = min(fit(:,it),[],1);
            disp(['[iter=' num2str(it, '%i') ']' ' fitness=' num2str(Ym) '   k=' mat2str(k_particles)]);

            if(display == 2)
%                 eva = evidence_accumulation(ones(1,nm),M_labels);
%                 imagesc(eva,[0,1]); 
                imagesc(M_labels); 
                pause(0.01);
            end
...
function Sigma_inv = wnancov(in,U)
    U = real(U);
    alpha_vec = sum(U,2);
    k = size(U,1);
    ...
    for i = 1:k
        if(alpha_vec(i) > 2)
            ...
            Sigma_i = 1-distmat(x_z_i',x_z_i',7);
            Sigma_i = 0.5*(Sigma_i' + Sigma_i );
             
            Sigma_inv(:,:,i) = Sigma_i \ eye(dim);
        else
            Sigma_inv(:,:,i) = eye(dim) \ eye(dim);
        end
    end
end
