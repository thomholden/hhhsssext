...
%ç gotolist shows at which position every node went
...
while pointer<=length(newlist) % Until the newlist is filled
   for i=1:curlength % for every father
       B=[];
       for j=1:length(p)
           if p(j)==current(i) & j~=root & p(j)~=0 % find his childrenõ
              B=[B,j];
           end
       end %At this point all the children of every father have been found and entered in vector Â
       m=[];
